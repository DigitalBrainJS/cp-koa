...
class CPKoaApplication extends Koa {
    ...
    Object.defineProperties(this.context, {
      ...
      run: {
        ...
        value(fn, options) {
          ...
          const promise = CPromise.run(fn, options);

          this.scope.onSignal((signal, {err, force}) => {
            if (signal !== CPromise.SIGNAL_CANCEL) return;
            promise.cancel(err, force);
          });

          const progressHandler = () => {
            promise.progress((value, data) => {
              shadow.scope.progress(value, data, promise);
            });
          }

          !shadow.scope.isCaptured && shadow.scope.onCapture(progressHandler);
  ...
  callback() {
    ...
    const handleRequest = (req, res) => {
      ...
      return promise.onCancel((err)=>{
        this.emit('cancel', err, ctx);
      }).finally(() => {
